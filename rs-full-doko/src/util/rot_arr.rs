...
#[derive(Clone, Debug)]
pub struct RotArr<D: Clone + Sized, const N: usize> {
    pub(crate) starting_i: usize,

    // Das Array, das von der Perspektive starting_i aus hier
    // gespeichert wird.
    data: [D; N],
}
...
impl<D: Clone + Sized, const N: usize> RotArr<D, N> {

    pub fn new_from_0(
        pov_i: usize,
        data_from_0: [D; N]
    ) -> Self {
        let new_data = std::array::from_fn(|k| {
            let i = (k + pov_i) % N;
            data_from_0[i].clone()
        });
    ...
    pub fn new_from_perspective(
        ...
        RotArr {
            starting_i: perspective_of_index,
            data: data_from_pov,
        }
    }
    ...
    where
        F: FnMut(usize, D) -> R,
    {
        let new_data = from_fn(|i| f(i, self.data[i].clone()));
    ...
    pub fn get_element_for_perspective(&self, i: usize) -> D {
        self.data[index_for_i(self.starting_i, i, N)].clone()
    }
...
impl<D: Copy + Add + Zero, const N: usize> RotArr<D, N> {
    ...
    pub fn add_other_in_place(&mut self, other: &RotArr<D, N>) {
        for i in 0..N {
            let i_self = index_for_i(self.starting_i, i, N);
