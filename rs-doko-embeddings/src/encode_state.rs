...
use rs_doko::hand::hand::hand_to_vec;
...
fn get_relative_player_index(
    current_player: DoPlayer,
    target_player: DoPlayer
) -> usize {
...
pub fn encode_card_or_none(card: Option<DoCard>) -> usize {
    match card {
        None => 0,
...
pub fn encode_state(
    ...
    // Phase muss nicht in irgendeiner Weise rotiert werden.
    ...
    let reservation_start_player = get_relative_player_index(
        current_player, obs.game_starting_player
    ) + 1;

    let mut trick_starting_players: [Option<usize>; 12] = [None; 12];

    for (i, trick) in obs.tricks.iter().enumerate() {
        trick_starting_players[i] = trick.map(|t| get_relative_player_index(current_player, t.start_player));
    }
    ...
    for i in 0..4 {
        ...
        for j in 0..12 {
            let mut hand_as_vec = hand_to_vec(current_hand);

            hand_as_vec.sort_by_key(|&card| -card_to_rank_in_normal_game(card));

            cards_on_hands[i * 12 + j] = hand_as_vec.get(j).copied();
        }
    }

    fn encode_relative_player(player: Option<usize>) -> usize {
        match player {
            None => 0,
            Some(player) => player as usize + 1
        }
    }

    let encoded_trick_starting_players: Vec<usize> = trick_starting_players
        .iter()
    ...
    let encoded_trick_cards: Vec<usize> = trick_cards
        .iter()
        .map(|&card| encode_card_or_none(card))
        .collect();

    let encoded_hand_cards: Vec<usize> = cards_on_hands
        .iter()
        .map(|&card| encode_card_or_none(card))
        .collect();
    ...
    return [
        vec![
            ...
            phase_num,
            // 1
            reservation_start_player,
        ],
        // 2 - 13
        encoded_trick_starting_players,
        // 14 - 59
        encoded_trick_cards,
        // ??
        encoded_hand_cards
...
pub fn encode_state_with_reservations(
    state: &DoState
) -> Vec<usize> {
    let obs = state.observation_for_current_player();
    ...
    fn encode_visible_reservation(reservation: Option<DoVisibleReservation>) -> usize {
        match reservation {
            None => 0,
    ...
    let encoded_reservations: Vec<usize> = reservations
        .iter()
...
mod tests {
    ...
    fn test_encode_random_state() {
        ...
        for i in 0..15 {
            doko
                .random_action_for_current_player(&mut random);
        }

        let encoded = encode_state(&doko);
        ...
        assert_eq!(encoded, vec![
            // 0 : Phase (PlayCard)
            DoPhase::PlayCard as usize,
            // 1 : Startspieler des Spiels (relativ zu aktuellem Spieler PLAYER_TOP)
            get_relative_player_index(PLAYER_TOP, PLAYER_LEFT) + 1,
            ...
            get_relative_player_index(PLAYER_TOP, PLAYER_TOP) + 1,
            // 4 : Startspieler des dritten Stichs (hier: niemand)
            get_relative_player_index(PLAYER_TOP, PLAYER_RIGHT) + 1,
