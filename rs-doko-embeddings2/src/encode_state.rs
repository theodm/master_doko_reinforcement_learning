...
fn encode_relative_player(player: Option<usize>) -> i64 {
    match player {
        None => 0,
...
fn encode_visible_reservation(reservation: Option<DoVisibleReservation>) -> i64 {
    match reservation {
        None => 0,
...
pub fn encode_state_with_reservations_copy(
    obs: &DoObservation,
    memory: &mut [i64]
) {
    ...
    memory[2..14].copy_from_slice(
        &encoded_trick_starting_players
    );
    memory[14..62].copy_from_slice(
        &encoded_trick_cards
    );
    memory[62..110].copy_from_slice(
        &encoded_hand_cards
    );
    memory[110..114].copy_from_slice(
...
pub fn encode_state_with_reservations(
    ...
    encode_state_with_reservations_copy(
        &obs,
        &mut memory
    );
